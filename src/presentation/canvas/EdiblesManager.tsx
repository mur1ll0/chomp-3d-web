...
export const EdiblesManager: React.FC<EdiblesManagerProps> = ({ chunks }) => {
  ...
  const interactableData = useMemo(() => {
    ...
    const edible = ediblesMap.get(interactableEdibleId);
  ...
  return (
    <group>
      {/* Plants (Instanced) */}
      <instancedMesh ref={plantMeshRef} args={[undefined, undefined, MAX_EDIBLES_PER_TYPE]} frustumCulled={false}>
        <dodecahedronGeometry args={[0.5, 0]} />
        <meshStandardMaterial color="#84cc16" emissive="#3f6212" emissiveIntensity={0.5} />
      </instancedMesh>

      {/* Meats (Instanced) */}
      <instancedMesh ref={meatMeshRef} args={[undefined, undefined, MAX_EDIBLES_PER_TYPE]} frustumCulled={false}>
      ...
      {interactableData && (
        <mesh
          ...
          scale={[interactableData.scale * 1.05, interactableData.scale * 1.05, interactableData.scale * 1.05]}
