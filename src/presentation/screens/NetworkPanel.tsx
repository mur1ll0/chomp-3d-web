        ...
        <div className="absolute bottom-12 right-0 bg-slate-900/95 backdrop-blur border border-slate-700 rounded-xl p-4 w-72 shadow-2xl flex flex-col gap-3 pointer-events-auto">
          ...
          {isGlobal && (
            <div className="bg-slate-800/80 border border-blue-500/30 rounded-lg p-3 text-center">
              ...
              <div className="text-lg font-bold text-blue-400">Peer-to-Peer</div>
              <div className="text-xs text-slate-500">{totalPlayers} jogador(es) na área</div>
            </div>
          )}
            ...
            <div className="border-t border-slate-700 pt-2">
              ...
              {packRole === 'member' && (
                <div className="text-[10px] text-slate-500 mb-1">
                  Líder: {packMembers.find(m => m.peerId !== PeerMesh.getOwnPeerId())?.playerName ?? '---'}
                </div>
              )}
                ...
                {packMembers.map(m => {
                  ...
                  return (
                    <div key={m.peerId} className="flex items-center gap-2 bg-slate-800/50 rounded-md px-3 py-1.5">
                      ...
                      {isMe && <span className="text-[10px] text-slate-500">(Você)</span>}
                      {isLeader && !isMe && (
                        <button
              ...
              {isLeader && otherPeers.length > 0 && (
                <div className="mt-2 pt-2 border-t border-slate-700/50">
                  <div className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Convidar Jogadores</div>
