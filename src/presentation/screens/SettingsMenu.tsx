...
export const SettingsMenu: React.FC<{ inGame?: boolean }> = ({ inGame }) => {
  ...
  return (
    <div
      className={`min-h-screen flex items-start sm:items-center justify-center ${inGame ? 'bg-transparent' : 'bg-slate-900 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-slate-800 to-slate-950'} text-slate-200 w-full p-4`}
      onMouseDown={(e) => {
        if (!rebindingAction) return;
    ...
    >
      <div ref={panelRef} className="max-w-2xl w-full bg-slate-800/90 backdrop-blur-md rounded-2xl shadow-2xl border border-slate-700/50 flex flex-col max-h-[85vh] overflow-y-auto p-4 sm:p-6 md:p-8">
